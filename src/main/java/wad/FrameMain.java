...
public class FrameMain extends JFrame {
	...
	public FrameMain(String configPath) {
		try {
			...
			handleInitializationError(e, configPath);
		}
	}

	private void handleInitializationError(Exception e, String configPath) {
		e.printStackTrace();
		
		StringBuilder errorMsg = new StringBuilder();
		errorMsg.append("Failed to load WAD configuration.\n\n");
		errorMsg.append("Config path: ").append(configPath).append("\n\n");
		errorMsg.append("Error: ").append(e.getMessage()).append("\n\n");
		
		// Add helpful hints based on error type
		String errorMessage = e.getMessage();
		if (errorMessage != null) {
			if (errorMessage.contains("Premature end of file")) {
				errorMsg.append("Hint: XML file may be corrupted or not ZIP-compressed.\n");
				errorMsg.append("The file should be named wad.xml.zip\n");
			} else if (errorMessage.contains("No such file") || errorMessage.contains("cannot find")) {
				errorMsg.append("Hint: Configuration or data file not found.\n");
				errorMsg.append("Check that file paths exist and are correctly formatted.\n");
			} else if (errorMessage.contains("encoding")) {
				errorMsg.append("Hint: File encoding issue detected.\n");
				errorMsg.append("Ensure XML files are UTF-8 encoded.\n");
			}
		}
		
		errorMsg.append("\nPlease check the README for configuration instructions.");
		
		JOptionPane.showMessageDialog(null, errorMsg.toString(), "WAD - Initialization Error",
